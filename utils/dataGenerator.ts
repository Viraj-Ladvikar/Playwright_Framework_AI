...
/**
 * RandomDataUtil class for generating random test data using Faker.
 */
export class RandomDataUtil {
  ...
  static getCurrentDate(): string {
    return faker.date.recent({ days: 1 }).toISOString().split('T')[0];
  }
  ...
  static generateUserUpdatePayload() {
    const user = this.generateUserPayload();
    return {
      ...user,
